...
class LunarCalenderViewController: BaseVc {
    ...
    var searchLunarCalener: [LunarCalenderModel] = []
...
extension LunarCalenderViewController {
    ...
    @IBAction func searchSubCategoryTFEditingChanged(_ sender: UITextField) {
        ...
        if searchText != "" {
            isSearchEnable = true
            searchLunarCalener = LunarCalener.filter({ (catgegory) -> Bool in
                let lunarName = ("\(catgegory.lunarName)").lowercased()
                return lunarName.contains(searchText)
            })
            lunarCalTableView.reloadData()
        } else {
...
extension LunarCalenderViewController: UITextFieldDelegate {
...
extension LunarCalenderViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return isSearchEnable == true ? searchLunarCalener.count + 1 : LunarCalener.count + 1
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0 {
            ...
            attr.text(isSearchEnable == true ? searchLunarCalener[indexPath.row - 1].lunarName : LunarCalener[indexPath.row - 1].lunarName,
                      attributes: [.textColor(UIColor.white),
                                   .font(UIFont.init(type: .PlayfairDisplay_Regular, size: 25)!)])
                .text("  \(isSearchEnable == true ? searchLunarCalener[indexPath.row - 1].lunarDate : LunarCalener[indexPath.row - 1].lunarDate)",
                    attributes: [.textColor(UIColor.white),
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if !StorageManager.isPremiumUser {
            ...
            if indexPath.row == 0 {
                let calVCnt = StoryboardScene.Main.LunarMoonPhasesViewController.instantiate()
                ...
                contentVCnt.purpose = .LunarCalendar
                contentVCnt.lunarCalendar = isSearchEnable == true ? searchLunarCalener[indexPath.row - 1] : LunarCalener[indexPath.row - 1]
